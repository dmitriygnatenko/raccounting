...
func TestCreateSession(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec(query).WithArgs(token, userID, expiresAt).WillReturnError(sqliteForeignKeyErr())
			},
		...
		},
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec(query).WithArgs(token, userID, expiresAt).WillReturnError(sqliteUniqueErr())
			},
			assertErr: func(t *testing.T, err error) { require.Error(t, err) },
		},
	}
