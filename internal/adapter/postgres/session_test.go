...
// errors rather than enforced by a real database.
func TestCreateSession(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec(query).WithArgs(token, userID, expiresAt).WillReturnError(pgErr(pgForeignKeyViolation))
			},
		...
		},
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec(query).WithArgs(token, userID, expiresAt).WillReturnError(pgErr(pgUniqueViolation))
			},
			assertErr: func(t *testing.T, err error) { require.Error(t, err) },
		},
	}
...
func TestFindSessionByToken(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			assertResult: func(t *testing.T, got model.Session) {
				require.Equal(t, model.Session{Token: token, UserID: userID, ExpiresAt: expiresAt}, got)
			},
			assertErr: func(t *testing.T, err error) { require.NoError(t, err) },
		},
		{
