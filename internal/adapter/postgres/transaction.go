...
// also in scope. Placeholders are numbered sequentially ($1, $2, ...) as args are appended, matching
// Postgres's convention (unlike sqlite/mysql's positional `?`).
func buildTransactionListWhere(filter model.TransactionListFilter) (string, []any) {
	...
	add := func(condFormat string, val any) {
		args = append(args, val)
		conditions = append(conditions, fmt.Sprintf(condFormat, len(args)))
	}

	if filter.DateFrom != nil {
		add("t.operation_at >= $%d", filter.DateFrom.Format(entity.DateLayout))
	}

	if filter.DateTo != nil {
		add("t.operation_at <= $%d", filter.DateTo.Format(entity.DateLayout))
	}
	...
	if filter.Type != nil {
		add("t.type = $%d", uint8(*filter.Type))
	}

	if filter.TagID != nil {
		add("EXISTS (SELECT 1 FROM transaction_tags tt WHERE tt.transaction_id = t.id AND tt.tag_id = $%d)", *filter.TagID)
	}

	if filter.Search != "" {
		like := "%" + filter.Search + "%"
		args = append(args, like)
		memoPlaceholder := len(args)
		args = append(args, like)
		categoryPlaceholder := len(args)
		conditions = append(conditions,
			fmt.Sprintf("(t.memo LIKE $%d OR c.name LIKE $%d)", memoPlaceholder, categoryPlaceholder),
		)
	}
...
func (s *Storage) ListTransactionsFiltered(
	...
	rows, err := s.DB.QueryContext(ctx,
		`SELECT `+transactionColumnsQualified+` FROM `+from+where+
			` ORDER BY t.operation_at DESC, t.id DESC LIMIT `+limitPlaceholder+` OFFSET `+offsetPlaceholder,
		pageArgs...,
	)
	if err != nil {
		return model.ListTransactionsFilteredResult{}, err
...
func (s *Storage) findTransactionTagIDs(ctx context.Context, transactionID uint64) ([]uint64, error) {
	rows, err := s.DB.QueryContext(ctx,
		`SELECT tag_id FROM transaction_tags WHERE transaction_id = $1`, transactionID,
	)
	if err != nil {
		return nil, err
	}
...
func setTransactionTags(ctx context.Context, tx *sql.Tx, transactionID uint64, tagIDs []uint64) error {
	if _, err := tx.ExecContext(ctx,
		`DELETE FROM transaction_tags WHERE transaction_id = $1`, transactionID,
	); err != nil {
		return err
	}
	...
	for _, tagID := range tagIDs {
		if _, err := tx.ExecContext(ctx,
			`INSERT INTO transaction_tags (transaction_id, tag_id) VALUES ($1, $2)`, transactionID, tagID,
		); err != nil {
			return err
		}
	}
...
func (s *Storage) CreateTransactionWithBalance(
	...
	if err = setTransactionTags(ctx, tx, id, req.TagIDs); err != nil {
		return model.Transaction{}, err
	}
