...
func (s *Storage) ExistsCurrency(ctx context.Context, code string) (bool, error) {
	...
	err := s.DB.QueryRowContext(ctx,
		`SELECT COUNT(*) FROM currencies WHERE code = $1`, code,
	).Scan(&n)
...
func (s *Storage) CreateCurrency(ctx context.Context, req port.CurrencyCreateRequest) error {
	...
	if _, err = tx.ExecContext(ctx, `UPDATE currencies SET is_default = FALSE`); err != nil {
		return err
	}
...
func updateCurrency(ctx context.Context, db dbtx, req port.CurrencyUpdateRequest) (model.Currency, bool, error) {
	...
	err = db.QueryRowContext(ctx,
		`SELECT `+currencyColumns+` FROM currencies WHERE code = $1`, req.Code,
...
func (s *Storage) DeleteCurrency(ctx context.Context, code string) (bool, error) {
	res, err := s.DB.ExecContext(ctx, `DELETE FROM currencies WHERE code = $1`, code)
	if err != nil {
