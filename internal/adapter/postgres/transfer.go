...
func (s *Storage) DeleteTransferWithBalance(ctx context.Context, id uint64) (bool, error) {
	...
	err = tx.QueryRowContext(ctx,
		`SELECT account_id, amount, transfer_transaction_id FROM transactions WHERE id = $1 FOR UPDATE`, id,
	...
	if transferTransactionID.Valid {
		...
		err = tx.QueryRowContext(ctx,
			`SELECT account_id, amount FROM transactions WHERE id = $1 FOR UPDATE`, otherID,
	...
	for _, l := range legs {
		if _, err = tx.ExecContext(ctx, `DELETE FROM transactions WHERE id = $1`, l.id); err != nil {
			return false, err
		}
